...
function App() {
  ...
  useEffect(() => {
    function get() {
      const latest_time =
        localStorage.getItem(`next_time${chooseClass}`) || undefined;
      getVideo(latest_time, logout ? undefined : token, videoClass[chooseClass])
        .then((res) => {
        ...
        switch (res.status_code) {
          case 0:
            ...
            localStorage.setItem(
              "videos",
              JSON.stringify([...videos, ...res.video_list])
            );
            localStorage.setItem(`next_time${chooseClass}`, res.next_time);
            setHaveVideo(true);
            break;
